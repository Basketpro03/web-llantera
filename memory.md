...
- [ ] Publicación en GoDaddy pendiente (todo el sitio ya es estático y autocontenido, salvo los CDN de Google Fonts y EmailJS).
  - **2026-07-13**: el usuario ya tiene el DOMINIO en GoDaddy; se le explicó que dominio ≠ hosting y se le dieron los pasos (cPanel → Administrador de archivos → `public_html`). Se creó `sitio_para_godaddy.zip` (3 MB) en la raíz del proyecto con SOLO los archivos del sitio: `index.html`, `favicon.png`, `logo_outline_navbar.png`, `llantera_buelna.png`, `nosotros_llantera.jpg`, `css/`, `js/`. Regenerar el ZIP si el sitio cambia antes de subirlo.
  - **CONFIRMADO (2026-07-13): el usuario SOLO tiene el dominio en GoDaddy, sin plan de hosting** (por eso no encontraba el "Administrador de archivos" — no existe en su cuenta). Se le recomendó Netlify gratis. Captura del usuario confirmó que en GoDaddy solo tiene el DOMINIO: **llantassato.com** (sin hosting GoDaddy).
  - **HALLAZGO CLAVE (2026-07-13, verificado por DNS/HTTP desde esta máquina):** `llantassato.com` YA está en línea sirviendo el **sitio viejo** ("Super Llantas Sato - Tienda Virtual", con la lista vieja de marcas: Nexen, Goodride, Sailun, BFGoodrich...; Google Analytics `G-7QDKHZW8SJ`; canonical `www.llantassato.com`). Nameservers: `ns1/ns2.dns-parking.com` (= **Hostinger**), IP `195.179.239.182` (Hostinger). Es decir: dominio registrado en GoDaddy pero delegado a Hostinger, donde alguien hospeda el sitio viejo.
  - Implicación: para publicar el sitio nuevo hay que (a) conseguir acceso a esa cuenta de Hostinger y reemplazar el sitio viejo ahí, o (b) cambiar nameservers/DNS desde GoDaddy hacia otro hosting (ej. Netlify), lo que desconecta el sitio viejo.
  - Reporte de "píxeles vacíos" arriba a la izquierda (2026-07-13): se verificó con Playwright (escalas 1x/1.25x/1.5x) que la esquina del sitio nuevo renderiza #111 uniforme sin defectos — NO es bug de la página; casi seguro es el redondeo de esquinas de ventana de Windows 11. Se le pidió confirmar maximizando la ventana; quedó sin confirmar.
  - **2026-07-21 — retomado.** El usuario confirmó explícitamente que quiere publicar en **Hostinger** (no Netlify) pero **todavía NO tiene acceso** a la cuenta de Hostinger que hospeda el sitio viejo (confirmado vía pregunta directa). Se le dieron pasos para recuperar acceso (buscar "Hostinger" en Gmail, probar recuperar contraseña en hpanel.hostinger.com con sus correos) o, si no aparece, comprar hosting nuevo en Hostinger y cambiar nameservers desde GoDaddy.
  - **2026-07-21 — INVESTIGADO A FONDO (vía capturas de pantalla del usuario en hPanel):** el usuario SÍ logró entrar a `hpanel.hostinger.com`, pero está en **"Modo de suplantación"** administrando la cuenta **`pollosato@gmail.com`** (nombre "Jesus") — confirmado por el usuario como cuenta de un **familiar/socio del negocio**, legítima. Esa cuenta administra el **dominio** `llantassato.com` (aparece en su sección "Dominios"), pero **NO tiene ningún plan de hosting activo** — la sección "Sitios web" redirige directo a `hpanel.hostinger.com/buy-hosting?emptyState=1` (estado vacío, invita a comprar).
  - **Confirmado con `curl` (2026-07-21) que el sitio viejo SIGUE en línea y activo**: `https://llantassato.com` responde `200 OK`, headers `Server: LiteSpeed`, `platform: hostinger`, `panel: hpanel`, IP `195.179.239.182` (mismo IP de la investigación anterior). Es decir, el sitio viejo vive en **otra cuenta de Hostinger distinta** a `pollosato@gmail.com` (probablemente la del diseñador/agencia original) — no tenemos ni tendremos acceso fácil a esa cuenta.
  - **PLAN RECOMENDADO Y ACORDADO (2026-07-21):** en vez de perseguir la cuenta vieja, comprar un plan de hosting **nuevo** directamente dentro de `pollosato@gmail.com` (que ya administra el DNS del dominio) — plan **"Single"** (~MX$24.99/mes, listado en `hpanel.hostinger.com/buy-hosting`) alcanza para un sitio estático simple como este. Al comprarlo y conectar el dominio `llantassato.com` desde la misma cuenta, Hostinger actualiza el DNS automáticamente (sin tocar nameservers en GoDaddy) y ya no depende de la cuenta vieja. Después: subir `sitio_para_hostinger.zip` al `public_html` de ese hosting nuevo (Administrador de archivos → Subir → Extraer).
  - **PENDIENTE (al retomar):** el usuario dijo "guarda todo" (2026-07-21) sin haber decidido aún si comprar el plan Single ahora o primero intentar contactar a quien dejó el sitio viejo para evitar el gasto — preguntarle en qué quedó esa decisión.
  - **Bug encontrado y corregido en esta sesión (2026-07-21):** `favicon.png` (referenciado en `index.html:7`) había desaparecido del disco — el archivo no existía en la carpeta del proyecto aunque `index.html` ya apuntaba a él (commit `fe02569` cambió el `<link rel="icon">` de `Logo Solo S.png` a `favicon.png`, pero el archivo nunca se recreó/commiteó). Se restauró copiando `Logo Solo S.png` → `favicon.png`. **Este archivo no está trackeado por git** (no aparece en `git status` ni en commits) — verificar que siga presente en el disco antes de cada publicación/ZIP, no asumir que git lo preserva.
  - **ZIP de publicación renombrado:** `sitio_para_godaddy.zip` → `sitio_para_hostinger.zip` (mismo contenido: `index.html`, `favicon.png`, `logo_outline_navbar.png`, `llantera_buelna.png`, `nosotros_llantera.jpg`, `css/`, `js/`; regenerado con `Compress-Archive` de PowerShell ya que `zip` no está disponible en Git Bash de esta máquina). El zip viejo `sitio_para_godaddy.zip` quedó obsoleto, se puede borrar. Regenerar este zip (con los mismos 7 paths) cada vez que cambie el sitio antes de subirlo.
